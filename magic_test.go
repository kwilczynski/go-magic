...
func TestMagic_Separator(t *testing.T) {
	...
	if n < 524 || n > 533 {
		// Older version of libmagic reports the results differently, but also latest version has been fixed
		// so it does not try to report text files as "data" any more, albeit it returns an extra separator
		// entry which causes an extra entry to be added.
